...
function count_parameters(x)
    if x isa AbstractArray
        if eltype(x) <: Complex
            return 2 * length(x)  # complex weight = real + imaginary
        else
            return length(x)
        end
    elseif x isa Number
        return x isa Complex ? 2 : 1
    elseif x isa NamedTuple
        return sum(count_parameters(v) for v in values(x); init = 0)
    elseif x isa Tuple
        return sum(count_parameters(v) for v in x; init = 0)
...
It loads the dataset using DatasetIO split data and evaluates only X_test/Y_test.
...
function evaluate_dataset(
    ...
    @info "Loaded evaluation data: X=$(size(X_test)), Y=$(size(Y_true))"

    @assert ndims(X_test) == 3 "X must be (nx, channels, samples). Got $(size(X_test))."
    @assert ndims(Y_true) == 3 "Y must be (nx, 1, samples). Got $(size(Y_true))."
    @assert size(X_test, 1) == size(Y_true, 1) "X/Y nx mismatch."
    @assert size(Y_true, 2) == 1 "Y must have one output channel."
    @assert size(X_test, 3) == size(Y_true, 3) "X/Y sample count mismatch."

    in_channels = size(X_test, 2)
    ...
    parameter_count = count_parameters(ps)
    @info "Parameter count=$parameter_count"

    # 4. Forward pass
    Y_pred, st = model_forward(model_name, model, X_test, grid_points, ps, st)
