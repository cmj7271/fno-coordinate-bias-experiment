...
# Helper to split data to train, valid, test
function split_dataset(X, Y, n_train::Int, n_valid::Int, n_test::Int)
    @assert size(X, 3) == n_train + n_valid + n_test
    @assert size(Y, 3) == n_train + n_valid + n_test

    train_range = 1:n_train
    valid_range = (n_train+1):(n_train+n_valid)
    test_range = (n_train+n_valid+1):(n_train+n_valid+n_test)

    return Dict(
        :X_train => X[:, :, train_range],
        :Y_train => Y[:, :, train_range],
        :X_valid => X[:, :, valid_range],
...
function generate_periodic_heat_dataset(config::Dict)
    ...
    n_train = Int(config_data[:train_samples])
    n_valid = Int(config_data[:valid_samples])
    n_test = Int(config_data[:test_samples])

    samples = n_train + n_valid + n_test
    ...
    return split_dataset(X, Y, n_train, n_valid, n_test)
end
