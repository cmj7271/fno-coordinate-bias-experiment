...
@testset "Test relative_l2" begin
    # Perfect match
    @test relative_l2(ones(5), ones(5)) ≈ 0.0
    
    # No match
    @test relative_l2(zeros(5), ones(5)) > 0.5 # Should be roughly 1.0
end
...
@testset "Test batch_relative_l2" begin
    ...
    y_pred = ones(5, 1, 3)
    y_true = ones(5, 1, 3)
    @test batch_relative_l2(y_pred, y_true) ≈ 0.0
    ...
    @test batch_relative_l2(y_pred, y_true) > 0.95
end
...
@testset "Test initial_sensitivity_error" begin
    ...
    y_pred = ones(5, 1, 3)
    y_true = ones(5, 1, 3)
    @test initial_sensitivity_error(y_pred, y_true) ≈ 0.0
    ...
    @test initial_sensitivity_error(y_pred, y_true) < 0.1
end

@testset "Test coordinate_benefit" begin
    # If errors are equal, benefit should be 0
    @test coordinate_benefit(0.5, 0.5) ≈ 0.0
    
    # If benefit is large (e.g., 0.5 - 0.0)
    @test coordinate_benefit(0.5, 0.0) > 0.4
end
